...
type FormData = Record<string, SkylarkObjectMetadataField>;
...
export interface AIFieldGeneration {
  ...
  populateAllFieldsUsingAiValues: (field?: string) => void;
  generateFieldSuggestions: (field: string) => void;
  fieldIsAiSuggestion: (field: string) => boolean;
}

export interface UseSkylarkObjectFormWithAutogeneratedValuesReturn
  extends UseFormReturn<FormData> {
...
export const useSkylarkObjectFormWithAutogeneratedValues = ({
  ...
  const { setValue, getValues, ...form } = useForm<FormData>(formProps);
  ...
  const generateFieldValuesWrapper = useCallback(
    (field: string, fieldsToAutoFill?: string[]) => {
  ...
  const populateFieldUsingAiValue = useCallback(
    (field: string) => {
      if (!hasAiSuggestions) {
        generateFieldValuesWrapper(field, [field]);
        return;
      }
  ...
  const populateAllFieldsUsingAiValues = useCallback(() => {
    ...
    if (!hasAiSuggestions) {
      generateFieldValuesWrapper("generic-all-fields-trigger", fields);
      return;
    }
