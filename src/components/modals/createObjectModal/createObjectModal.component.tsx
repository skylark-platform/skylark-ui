...
const CreateObjectModalBody = forwardRef(
  (
    ...
    useEffect(() => {
      const modal = isCreateTranslationModal
        ? SEGMENT_KEYS.modals.createTranslation
        : SEGMENT_KEYS.modals.createObject;
      segment.track(modal.open, { createTranslation });
    }, [createTranslation, isCreateTranslationModal]);
    ...
    const { createObject, isPending: isCreatingObject } = useCreateObject({
      ...
      onSuccess: (object) => {
        ...
        segment.track(SEGMENT_KEYS.modals.createObject.created, { object });
        closeModal();
      },
      onError: (error) => {
        toast.error(
    ...
    const {
      ...
      onSuccess: (object) => {
        ...
        segment.track(SEGMENT_KEYS.modals.createTranslation.created, {
          object,
        });
        closeModal();
      },
                ...
                {/* <div>
                  ...
                  {dimensions?.map((dimension) => (
                    <DimensionMultiSelect
                      key={dimension.uid}
                      dimension={dimension}
                      selected={getValues(`dimension.${dimension.slug}`) || []}
                      onChange={(slug, values) => {
                        setValue(`dimension.${slug}`, values);
                      }}
                    />
                  ))}
                </div> */}
