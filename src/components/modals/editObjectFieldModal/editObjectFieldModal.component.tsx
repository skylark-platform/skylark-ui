...
export type EditObjectFieldModalForm = {
  ...
  // | { type: "relationship"; relationship: SkylarkObjectRelationship }
  // config: ParsedSkylarkObjectConfigFieldConfig;
  // & Partial<ParsedSkylarkObjectTypeRelationshipConfigurations[""]>;
};
...
const EditObjectFieldModalBody = forwardRef(
  (
    ...
    const {
      ...
      formState,
      setValue,
    } = useForm<EditObjectFieldModalForm>({
      defaultValues: initialValues || undefined,
    });

    console.log("dirty", formState.dirtyFields, formState.isDirty);
      ...
      <>
        ...
        >
          <div className="w-full h-full flex flex-grow gap-4 flex-col">
                ...
                render={({ field }) => {
                  return (
                    <EnumSelect
                      label="Enum"
                      labelVariant="form"
                      selected={values.field.originalType}
                      onChange={(val) => {
            ...
            {["string"].includes(values.field?.type) && (
              <div className="flex flex-col gap-2">
                <PanelSeparator className="mb-4" />
                <PanelSectionTitle text="Field config" />
                {values.field?.type === "string" && (
                  <Controller
                    name="field.config.fieldType"
                    control={control}
                    render={({ field }) => {
                      return (
                        <Select
