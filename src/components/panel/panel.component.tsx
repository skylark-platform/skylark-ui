...
import { createIntegrationServiceObj } from "src/components/integrations";
import {
  Tabs,
...
import { createIntegrationUploadQueryKeyBase } from "src/hooks/integrations/useGenerateIntegrationUploadUrl";
...
export const Panel = ({
  ...
  useQuery({
    queryKey: [
      ...createIntegrationUploadQueryKeyBase("video", "wowza"),
      uid,
      objectType,
      ,
    ...
    queryFn: async () => {
      return integrationServiceRequest(`/refresh/video/wowza`, {
        body: { external_id: data?.metadata.external_id },
        method: "POST",
      });
    },
    enabled:
      ...
      Boolean(
        data?.metadata.external_id &&
          data.metadata.provider === "WOWZA" &&
          data.metadata.status &&
          data.metadata.status !== "FINISHED",
      ),
